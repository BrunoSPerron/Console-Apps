...
namespace TacticRoguelikeRpg
{
    public class ExtendedConsole
    {
        ...
        public static void Update(int left = 0, int top = 0, int width = -1, int height = -1)
        {
            ...
            //La commande win32 utilisé plus loin copy l'info des cases situé dans les limites d'un RECT (struct) d'un tableau de CHAR_INFO (struct) aux même position dans le buffer de la console
            //crée un tableau en conséquence et le rempli avec les characters à afficher
        ...
        public static CHAR_INFO GetCHARINFOAtPosition(int _x, int _y, int _layer = -1)
        {
            ...
            {
                if (Layers[_layer][_x, _y].charData != null)
                {
                    toReturn.charData = Layers[_layer][_x, _y].charData;
                    toReturn.attributes = Layers[_layer][_x, _y].attributes;
                    return toReturn;
                }
            }
